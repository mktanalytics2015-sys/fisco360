import { CheckCircle, AlertTriangle, FileText, Calendar, Clock, BadgePercent, Building, ChevronDown, ChevronUp, Printer, Calculator, TrendingUp, Layers, Info } from 'lucide-react';
  ...
  return (
    <TooltipProvider>
      <div className="space-y-6 animate-fade-in">
        ...
        {/* Regimes de Tributação Específicos */}
        <div className="card-elevated overflow-hidden border-2 border-primary/20">
          <button
            onClick={() => toggleSection('regimes')}
          ...
          >
            ...
            {expandedSections.regimes ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
          </button>
          
          {expandedSections.regimes && (
            <div className="p-4 space-y-4">
              {/* Imposto Industrial */}
              <div className="p-4 rounded-lg border border-blue-200 bg-blue-50/50">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <span className="text-lg font-bold text-blue-800">Imposto Industrial (II)</span>
                    <span className="text-xs font-bold bg-blue-200 text-blue-900 px-2 py-1 rounded">
                      {industrialTaxRegime.grupo}
                    </span>
                  </div>
                  <span className="text-sm font-semibold text-blue-700">
                    {(industrialTaxRegime.taxa * 100).toFixed(1)}%
                  </span>
                </div>
                <p className="text-sm text-blue-700 mb-2">{industrialTaxRegime.name}</p>
                <p className="text-xs text-blue-600 mb-3">{industrialTaxRegime.descricao}</p>
                <div className="p-2 rounded bg-blue-100/80 font-mono text-xs text-blue-900">
                  <span className="font-semibold">Fórmula:</span> {industrialTaxRegime.formula}
                </div>
              </div>
              ...
              <div className="p-4 rounded-lg border border-green-200 bg-green-50/50">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <span className="text-lg font-bold text-green-800">IVA</span>
                    <span className="text-xs font-bold bg-green-200 text-green-900 px-2 py-1 rounded">
                      {ivaRegime.id === 'iva_geral' ? 'GERAL' : ivaRegime.id === 'iva_simplificado' ? 'SIMPLIFICADO' : 'EXCLUSÃO'}
                    </span>
                  </div>
                  <span className="text-sm font-semibold text-green-700">
                    {ivaRegime.taxa > 0 ? `${(ivaRegime.taxa * 100).toFixed(0)}%` : 'Isento'}
                  </span>
                </div>
                <p className="text-sm text-green-700 mb-2">{ivaRegime.name}</p>
                <p className="text-xs text-green-600 mb-3">{ivaRegime.descricao}</p>
                <div className="p-2 rounded bg-green-100/80 font-mono text-xs text-green-900 mb-3">
                  <span className="font-semibold">Fórmula:</span> {ivaRegime.formula}
                </div>
                <div className="space-y-1">
                  <p className="text-xs font-medium text-green-800">Obrigações:</p>
                  <ul className="space-y-1">
                    {ivaRegime.obrigacoes.map((obr, idx) => (
              ...
              <div className="p-4 rounded-lg border border-amber-200 bg-amber-50/50">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-lg font-bold text-amber-800">IRT - Imposto sobre Rendimentos do Trabalho</span>
                </div>
                <p className="text-sm text-amber-700 mb-2">
                  Imposto progressivo sobre salários (retenção mensal)
                </p>
                <div className="p-2 rounded bg-amber-100/80 font-mono text-xs text-amber-900 mb-2">
                  <span className="font-semibold">Fórmula:</span> IRT = (Salário Bruto × Taxa) - Dedução
                </div>
                <div className="p-2 rounded bg-amber-200/50 text-xs text-amber-800">
                  <span className="font-semibold">⚠️ Isenção:</span> Salários até 150.000 Kz estão isentos de IRT
                </div>
              </div>
            </div>
          )}
