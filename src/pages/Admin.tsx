...
import { Scale, Users, BarChart3, ArrowLeft, Shield, Crown, Ban, CheckCircle } from 'lucide-react';
...
const Admin = () => {
  ...
  const [stats, setStats] = useState({ total: 0, premium: 0, active: 0, simulationsToday: 0 });
  ...
  const fetchUsers = async () => {
    ...
    if (data) {
      ...
      setStats({
        ...
        simulationsToday: data.reduce((sum, u) => sum + u.simulations_this_month, 0),
      });
    }
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-2xl font-display font-bold">Painel de Administração</h1>
            </div>
      ...
      <main className="container mx-auto px-4 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { icon: BarChart3, label: 'Simulações (mês)', value: stats.simulationsToday, color: 'text-primary' },
          ].map((stat, i) => (
                  ...
                  <tr key={u.id} className="border-b border-border hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3">
                      <div className="flex gap-2 justify-end">
                        <Button size="sm" variant="outline" onClick={() => togglePremium(u.user_id, u.subscription_status)} title={u.subscription_status === 'premium' ? 'Remover Premium' : 'Dar Premium'}>
                          <Crown className="w-4 h-4" />
                        </Button>
                        <Button size="sm" variant={u.is_active ? 'destructive' : 'default'} onClick={() => toggleActive(u.user_id, u.is_active)} title={u.is_active ? 'Desactivar' : 'Activar'}>
